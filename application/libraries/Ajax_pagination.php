...
class Ajax_pagination{
   ...
   //tambahan parameter   
   var $param = '';//for view by get param segment
   var $limit_class = '';
      ...
      if ($this->show_count)
      {
		...
		for ($loop = $start -1; $loop <= $end; $loop++)
		{
			...
			if ($i >= 0)
			{
				...
				{
					$output .= $this->cur_tag_open.'<li class="active"><a href="#">'.$loop.'</a></li>'.$this->cur_tag_close; // Current page
				}
				else
				{
					$n = ($i == 0) ? '' : $i;
	...
	function getAJAXlink($count, $text) {
        ...
        if( $this->div == '')
            return '<a href="'. $this->anchor_class . ' ' . $this->base_url . $count . '"></a>';
            
        $pageCount = $count?$count:0;
      
        //parameter post to ajax
        $this->additional_param = "{'page' : $pageCount,'param': '".$this->param."'}";	
        //ajax fungction        
		return '<li><a href="javascript:void(0)" onclick="pagination(\''.$this->base_url.$pageCount.'\',\'\',\'historyord\')">'.$text.'</a></li>';   
