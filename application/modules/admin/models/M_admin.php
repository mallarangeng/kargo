...
class M_admin extends CI_Model{
	...
	function report_filter($shift,$from,$to,$sumall=false){
		$this->db->where_in('A.id_shift',$shift);
		$this->db->where('DATE_FORMAT(A.date_add,"%Y-%m-%d") >=',$from);		
		$this->db->where('DATE_FORMAT(A.date_add,"%Y-%m-%d") <=',$to);
		//$this->db->group_by('DATE_FORMAT(A.date_add,"%Y-%m-%d")');
		...
		*/
		if ($from != $to){
			$shiftmax_timefrom = $this->session->userdata('shiftmax_timefrom');//get max time from db
		...
		(!$sumall) ? $this->db->group_by('A.id_shift') : '';
		$this->db->order_by('A.date_add','asc');
		$sql =  $this->get_rekap_order();		
		return $sql;
	}
	
	function report_id_order($shift,$from,$to){		
		$this->db->select('id_order');
		$this->db->from('tb_order');
		$this->db->where('active',1);
		$this->db->where('deleted',0);
		$this->db->where_in('id_shift',$shift);
		$this->db->where('DATE_FORMAT(date_add,"%Y-%m-%d") >=',$from);		
		$this->db->where('DATE_FORMAT(date_add,"%Y-%m-%d") <=',$to);		
		/*
		 * tidak menampailkan
		* jika shift tiga pada tanggal yg sama atau diluar waktu 23:00 - 07:00
		*/
		if ($from != $to){
			...
			//jika memilih shift 3 tanggal from dan to tidak boleh sama
			$this->db->where('DATE_FORMAT(date_add,"%Y-%m-%d %H:%i:%s") >', $from.' '.$shiftmax_timefrom);
			$this->db->where('DATE_FORMAT(date_add,"%Y-%m-%d %H:%i:%s") <', $to.' '.$shiftmax_timefrom);
		}
		$this->db->order_by('id_order','asc');
		return $this->db->get()->result();
	}
	/*
	 * digunakan untuk laporan general harian pershift
	 */
	function report_general($shift,$from,$to,$sum=false){
		...
		*/
		...
		$sql =  ($sum) ? $this->get_rekap_order() : $this->get_order(array('A.active'=>1));//use for report rekap
		return $sql;
	}
	...
	function id_last_order(){
		$sql = $this->db->query('SELECT MAX(id_order) as id_order from tb_order where deleted=0 and active=1')->result();
		return $sql[0]->id_order;
	}
	function get_last_ordet($id){
		$this->db->select('id_order_detail,qty_print');
		$this->db->from('tb_order_detail');
